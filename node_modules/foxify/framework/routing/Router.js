...
const EMPTY_HANDLE = {
    handlers: [
        new exceptions_1.Encapsulation(foxify_not_found),
    ],
    options: OPTIONS,
    params: {},
};
const pathMatchesMiddleware = (path, middleware) => {
...
const getWildcardNode = (layer, method, path, len) => {
    if (layer === null)
        return EMPTY_HANDLE;
    const decoded = utils_1.decodeURIComponent(path.slice(-len));
    if (decoded === null)
        return EMPTY_HANDLE;
...
const getClosingParenthensePosition = (path, idx) => {
...
class Router {
    constructor(prefix = "") {
        ...
        this.allowUnsafeRegex = false;
        this._next = (req, res, handlers, index = 0) => {
            const next = () => this._safeNext.run(req, res, handlers, index + 1);
            res.next = next;
            handlers[index].run(req, res, next);
        };
        this._safeNext = new exceptions_1.Encapsulation(this._next);
        ...
        httpMethods_1.default.forEach((method) => {
            ...
            this[methodName] = (path, opts, ...handlers) => this.on(method, path, opts, ...handlers);
        });
    }
    _on(method, path, opts = OPTIONS, handlers) {
        ...
        assert(httpMethods_1.default.indexOf(method) !== -1, `Method "${method}" is not an http method.`);
        this.routes.push({ method, path: `${this.prefix}${path}`, opts, handlers, middlewares: [] });
        return this;
    }
    _insert(method, path, kind, options = OPTIONS, params = [], handlers = [], middlewares = [], regex) {
        const prettyPrint = handlers.length !== 0;
        handlers = middlewares
            .concat(params.filter((param) => this.params[param] !== undefined)
        ...
        while (true) {
            ...
            if (len < prefixLen) {
                node = new Layer(prefix.slice(len), currentNode.children, currentNode.kind, currentNode.regex, params, new Layer.Handlers(currentNode.handlers));
                ...
                if (len === pathLen) {
                    assert(currentNode.getHandler(method).handlersLength === 0, `Method "${method}" already declared for route "${route}"`);
    ...
    _use(...handlers) {
        handlers = utils_1.array.compact(handlers);
        // handler validation
        handlers.forEach((handler) => assert(utils_1.isHandler(handler), "Handler should be a function"));
        const path = "*";
        const index = this.middlewares.findIndex((middleware) => path === middleware.path);
        if (index === -1)
            this.middlewares.push({ path, handlers });
        else
            this.middlewares[index].handlers = handlers.concat(this.middlewares[index].handlers);
        return this;
    }
    initialize(app) {
        ...
        this._use(middlewares_1.init(app));
        const middlewares = this.middlewares.reduce((prev, { path, handlers }) => {
            httpMethods_1.default.forEach((method) => prev.push({
                path,
                handlers: [],
                opts: OPTIONS,
                method,
        ...
        let routes = this.routes.reduce((prev, { method, path, opts, handlers }) => {
            ...
            if (schema)
                schema.response = utils_1.object.mapValues(schema.response || {}, (value) => fastStringify(value));
            ...
            const matchedMiddlewares = middlewares
                ...
                .reduce((prev, { middlewares }) => prev.concat(middlewares), []);
        ...
        if (filteredMiddleware)
            routes = filteredMiddleware.concat(routes);
        ...
        routes
            ...
            for (let i = 0, len = path.length; i < len; i++)
                ...
                if (path.charCodeAt(i) === 58) {
                    ...
                    if (i === len)
                        return this._insert(method, path.slice(0, i), nodeType, opts, params, handlers, middlewares, regex);
                    ...
                    params.push("*");
                    return this._insert(method, path.slice(0, len), 2 /* MATCH_ALL */, opts, params, handlers, middlewares, null);
                }
            if (!this.caseSensitive)
    ...
    on(method, path, opts, ...handlers) {
        if (utils_1.function.isFunction(opts) || Array.isArray(opts)) {
            handlers = [opts].concat(handlers);
            opts = {};
        }
        handlers = utils_1.array.deepFlatten(handlers);
        handlers = utils_1.array.compact(handlers);
        // path validation
    ...
    route(path) {
        const ROUTE = httpMethods_1.default.reduce((prev, method) => {
            const methodName = method.toLowerCase();
            if (prev[methodName])
                throw new Error(`Method already exists: ${methodName}`);
            prev[methodName] = (opts, ...handlers) => {
    ...
    all(path, opts, ...handlers) {
        return this.on(httpMethods_1.default, path, opts, ...handlers);
    }
    use(path, ...handlers) {
        if (!utils_1.string.isString(path)) {
        ...
        if (index === -1)
            this.middlewares
                .push({ path: middlewarePrefix, handlers: handlers });
        else
            this.middlewares[index].handlers.push(...handlers);
        ...
        routers.forEach((router) => {
            ...
            this.routes = this.routes.concat(router.routes.map((route) => (Object.assign({}, route, { path: `${prefix}${route.path}` }))));
    ...
    param(param, handler) {
        ...
        // handler validation
        assert(utils_1.isHandler(handler), "Handler should be a function");
    ...
    lookup(req, res) {
        const handle = this.find(req.method, req.path);
    ...
    find(method, path) {
        ...
        while (true) {
            ...
            if (kind === 1 /* PARAM */) {
                ...
                if (i > maxParamLength)
                    return EMPTY_HANDLE;
                decoded = utils_1.decodeURIComponent(path.slice(0, i));
                if (decoded === null)
                    return EMPTY_HANDLE;
            ...
            // wildcard route
            if (kind === 2 /* MATCH_ALL */) {
                decoded = utils_1.decodeURIComponent(path);
                if (decoded === null)
                    return EMPTY_HANDLE;
                params[pindex] = decoded;
                currentNode = node;
...
Router.isRouter = (arg) => arg instanceof Router;
